...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		int	__expected;
		vector <string>	codersAttempted;
vector <string>	codersChallenged;

		__in >> __expected >> codersAttempted >> codersChallenged;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << codersAttempted << ", " << codersChallenged << std::endl;
		std::cout.flush();

		__timer_start();

		SRMChallengePhase __object;
		int __actual = __object.countWays(codersAttempted, codersChallenged);
