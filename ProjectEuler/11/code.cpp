...
const int inf=1e9,maxn=22,mo=0;
...
//*/
LL n=20,a[maxn][maxn],ans;
int main(){
	rep(i,1,n)rep(j,1,n)a[i][j]=read();
	rep(i,1,n)rep(j,1,n-3){
		LL tmp=1;
		rep(k,0,3)tmp*=a[i][j+k];
		up(ans,tmp);
	}
	rep(j,1,n)rep(i,1,n-3){
		LL tmp=1;
		rep(k,0,3)tmp*=a[i+k][j];
		up(ans,tmp);
	}
	rep(i,1,n-3)rep(j,1,n-3){
		LL tmp=1;
		rep(k,0,3)tmp*=a[i+k][j+k];
		up(ans,tmp);
	}
	rep(i,1,n-3)rep(j,4,n){
		LL tmp=1;
		rep(k,0,3)tmp*=a[i+k][j-k];
		up(ans,tmp);
	}
