...
int main() {
	freopen("drink.in","r",stdin);freopen("d1.out","w",stdout);
	scanf("%d%d%d",&n,&k,&p);
	rep(i,1,n)scanf("%d",&h[i]);h1=h[1];
	sort(h+1,h+1+n,comp);
	while(h[n]<h1)--n;--n;
	if(n<=k){
		Decimal ans=h1;
		per(i,n,1)ans=(ans+Decimal(h[i]))/2;
		cout<<ans.to_string(p*6/5)<<endl;
	}else if(k==1){
		Decimal ans=h1,sum=h1;
		rep(i,1,n)sum+=Decimal(h[i]),ans=max(ans,sum/(i+1));
		cout<<ans.to_string(p*6/5)<<endl;
	}else{
		rep(i,1,n+1)f[0][i]=h1;
		rep(i,1,k){
			per(j,n,1){
				Decimal sum=0;
				rep(p,j+1,n+1){
					sum+=h[p-1];
					f[i][j]=max(f[i][j],(f[i-1][p]+sum)/(p-j+1));
				}
			}
		}
		Decimal ans=h1;
		rep(i,1,k)ans=max(ans,f[i][1]);
		cout<<ans.to_string(p*6/5)<<endl;
	}
	return 0;
}
