...
const int inf=1e9+1,maxn=5e5+10,mo=0,maxv=2e6+10;
...
bool check(){
	int l=0,r=inf;
	rep(i,1,n)if(mark[i])l=max(l,a[i].l),r=min(r,a[i].r);
	return l<=r;
}
int getans(){
	int maxx=0,minn=inf;
	rep(i,1,n)if(mark[i])maxx=max(maxx,a[i].r-a[i].l),minn=min(minn,a[i].r-a[i].l);
	return maxx-minn;
}
void dfs(int k,int cnt){
	if(k==n+1){
		if(cnt==m&&check())ans=min(ans,getans());
		return;
	}
	dfs(k+1,cnt);
	mark[k]=1;dfs(k+1,cnt+1);mark[k]=0;
}

int main(){
	freopen("interval.in","r",stdin);freopen("brute.out","w",stdout);
	n=read();m=read();
